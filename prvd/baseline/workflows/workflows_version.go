...
var versionBaselineWorkflowCmd = &cobra.Command{
	Use:   "version",
	Short: "Version a baseline workflow",
	Long:  `Version an existing baseline workflow`,
	Run:   versionWorkflow,
}
...
func versionWorkflowRun(cmd *cobra.Command, args []string) {
	...
	workflow, err := baseline.GetWorkflowDetails(*token.AccessToken, workflowID, map[string]interface{}{})
	if err != nil {
		fmt.Printf("failed to version workflow; %s", err.Error())
		os.Exit(1)
	}
	...
	w, err := baseline.VersionWorkflow(*token.AccessToken, workflowID, params)
	if err != nil {
		fmt.Printf("failed to version workflow; %s", err.Error())
		os.Exit(1)
	}
...
func init() {
	...
	versionBaselineWorkflowCmd.Flags().StringVar(&name, "name", "", "name of the baseline workflow")
	versionBaselineWorkflowCmd.Flags().StringVar(&description, "description", "", "description of the baseline workflow")
	versionBaselineWorkflowCmd.Flags().StringVar(&version, "version", "", "version of the baseline workflow")
