...
var initBaselineWorkstepCmd = &cobra.Command{
	Use:   "init",
	Short: "Initialize axiom workstep",
	Long:  `Initialize and configure a new axiom workstep`,
	Run:   initWorkstep,
}
...
func initWorkstepRun(cmd *cobra.Command, args []string) {
	...
	ws, err := axiom.CreateWorkstep(*token.AccessToken, workflowID, params)
	if err != nil {
		fmt.Printf("failed to initialize workstep; %s", err.Error())
		os.Exit(1)
	}
...
func init() {
	...
	initBaselineWorkstepCmd.Flags().StringVar(&name, "name", "", "name of the axiom workstep")
	initBaselineWorkstepCmd.Flags().StringVar(&description, "description", "", "description of the axiom workstep")
	initBaselineWorkstepCmd.Flags().BoolVar(&requireFinality, "require-finality", false, "require finality on the axiom workstep")
